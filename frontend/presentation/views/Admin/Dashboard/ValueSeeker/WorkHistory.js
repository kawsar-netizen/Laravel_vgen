...
export default function WorkHistory({ profile }) {
  ...
  const loadWorkHistoryList = () => {
    getData(
      `/admin/seeker-work-history/${profile?.basicInformation?.seekerId}?page=${page}`,
      adminToken
    ).then((result) => {
  ...
  const loadSingleWorkDetails = (id) => {
    getData(`/admin/seeker-work-details/${id}`, adminToken).then((result) => {
      let responseJSON = result;
  ...
  const loadSingleInvoice = (id) => {
    getData(`/admin/seeker-work-invoice/${id}`, adminToken).then((result) => {
      let responseJSON = result;
      console.log(responseJSON);
      if (responseJSON.success) {
  ...
  return (
    <>
            ...
            {workLists?.data?.map((work) => (
              <TableRow key={work.workId}>
                ...
                <TableCell align="center">
                  <Typography className={classes.regularData}>
                    {work?.generatorFirstName?.concat(
                      " " + work?.generatorLastName
                    )}
                  </Typography>
                </TableCell>
                <TableCell align="center">
      ...
      <div style={{ marginTop: 40 }}>
        {showWorkDetails && (
          <WorkDetails singleWorkDetails={singleWorkDetails} />
        )}
      </div>
    </>
  );
