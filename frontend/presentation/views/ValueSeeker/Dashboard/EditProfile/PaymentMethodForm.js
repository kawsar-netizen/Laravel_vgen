...
const PaymentMethodForm = () => {
  ...
  const getBankList = () => {
    ...
    getData(`/seeker/show-bank`, token).then((result) => {
      let response = result;
  ...
  const getMobileBankList = () => {
    const token = cookie.get("seekerUser");
    getData(`/seeker/show-mobile-bank`, token).then((result) => {
      let response = result;
  ...
  const deleteBankDetails = (id) => {
    const token = cookie.get("seekerUser");
    getData(`/seeker/remove-bank/${id}`, token).then((res) => {
      let response = res;
      if (response) {
        getBankList();
      }
  ...
  const deleteMobileBankDetails = (id) => {
    const token = cookie.get("seekerUser");
    getData(`/seeker/remove-mobile-bank/${id}`, token).then((res) => {
      let response = res;
      if (response) {
        getMobileBankList();
      }
  ...
  const handleFormSubmit = async (event) => {
    ...
    sendData(`/seeker/add-bank`, token, editProfileData).then((result) => {
  ...
  const handleMobileBankSubmit = async (event) => {
    ...
    console.log(editProfileData);
    sendData(`/seeker/add-mobile-bank`, token, editProfileData).then(
      (result) => {
        let responseJSON = result;
        if (responseJSON.Bank) {
          console.log(responseJSON);
