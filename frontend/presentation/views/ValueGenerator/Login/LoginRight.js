...
const LoginRight = () => {
  ...
  const login = async (event) => {
    ...
    let result = await fetch(`${server}/generator/login`, {
      method: "POST",
      headers: {
    ...
    if (result.success === true) {
      {
        isRemembered
          ? localStorage.setItem("generatorUser", JSON.stringify(result))
          : sessionStorage.setItem("generatorUser", JSON.stringify(result));
      }
      cookie.set("generatorUser", result.token);
      router.push("/valuegenerator/dashboard");
      console.log(result);
      setLoading(false)
      setSuccess(true)
  ...
  const passwordHandle = async (e) => {
    ...
    let result = await fetch(`${server}/generator/forgot-password`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Accept: "application/json",
